...
int folhas(PONT *raiz){
	if(raiz == NULL) return 0;
	int e = folhas(raiz->esq);
	int d = folhas(raiz->dir);
...
void bigger_smaller_trees(PONT raiz, TIPOCHAVE * max, TIPOCHAVE * min){
	
	if(raiz == NULL) return;
	bigger_smaller_trees(raiz->esq ,&max,&min);
	...
	bigger_smaller_trees(raiz->dir ,&max,&min);
}
